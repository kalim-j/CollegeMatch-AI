...
import { saveAndSendOTP } from '@/lib/otp';
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      /* Generate OTP + send Firebase email + save to Firestore
         saveAndSendOTP returns the OTP string */
      const otp = await saveAndSendOTP(cred.user);

      /* Store OTP in sessionStorage as screen fallback */
      sessionStorage.setItem(`otp_${cred.user.uid}`, otp);
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, 'users', cred.user.uid), {
        ...
        isNewUser: true,
        shownWelcome: false,
        emailVerified: true, /* Google accounts verified */
      }, { merge: true });

      /* Google users skip OTP — go straight to discover */
      ...
      <div className="auth-card-container relative z-10 flex w-full max-w-[1000px] bg-white/70 dark:bg-[#0f122b]/40 backdrop-blur-2xl rounded-[32px] border border-white/50 dark:border-white/5 shadow-2xl overflow-hidden flex-col md:flex-row">
        ...
        <div className="auth-branding-section flex-1 p-12 flex flex-col justify-center bg-gradient-to-br from-indigo-50/50 to-teal-50/50 dark:from-indigo-500/10 dark:to-teal-500/5">
          ...
          <p className="text-[16px] text-slate-600 dark:text-white/60 leading-[1.6] max-w-[380px] m-0 mb-10">
            Discover 500+ top colleges and find the one that fits your dreams — free.
          </p>
          ...
          <div className="inline-flex items-center gap-2.5 bg-white/50 dark:bg-white/5 border border-slate-200/50 dark:border-white/10 rounded-2xl px-5 py-3 max-w-fit backdrop-blur-md">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 dark:text-white/40 m-0 mt-0.5 uppercase tracking-wider">
                Free forever · No spam · Data is private
              </p>
            </div>
          </div>
        </div>
        ...
        {/* RIGHT / BOTTOM - Form */}
        <div className="auth-form-section flex-[0_0_440px] p-10 flex flex-col justify-center" style={{ background: isDark ? '#0d1230' : 'white' }}>
          <h2 style={{ color: isDark ? 'white' : '#1a1340', fontSize: '24px', fontWeight: 'bold', marginBottom: '8px' }}>
            Create free account
          </h2>
          <p style={{ color: isDark ? 'rgba(255,255,255,0.55)' : '#6b6894', fontSize: '14px', marginBottom: '24px' }}>
            Join 10,000+ students finding their dream college
          </p>

          <button onClick={handleGoogle} disabled={busy} className="w-full p-[14px_16px] rounded-2xl border border-slate-200 dark:border-white/10 bg-white/80 dark:bg-white/5 text-slate-800 dark:text-white text-[14px] font-semibold flex items-center justify-center gap-3 mb-5 transition-all hover:bg-slate-50 dark:hover:bg-white/10 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed shadow-sm">
          ...
          <form onSubmit={handleRegister}>
            <div className="mb-3">
              <label style={{ color: isDark ? '#a89ef8' : '#534AB7' }} className="block text-[11px] font-semibold uppercase tracking-wider mb-1.5">Full name</label>
            ...
            <div className="mb-3">
              <label style={{ color: isDark ? '#a89ef8' : '#534AB7' }} className="block text-[11px] font-semibold uppercase tracking-wider mb-1.5">Password</label>
              <div className="relative">
                <input type={showPwd ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="Min 6 characters" required style={{ background: isDark ? 'rgba(255,255,255,0.06)' : '#f0eeff', color: isDark ? 'rgba(255,255,255,0.9)' : '#1a1340', border: isDark ? '1px solid rgba(255,255,255,0.10)' : '1px solid rgba(127,119,221,0.2)' }} className="w-full p-[12px_44px_12px_16px] rounded-2xl text-[14px] outline-none transition-all focus:border-indigo-400 focus:ring-2 focus:ring-indigo-400/20 placeholder:text-slate-400 dark:placeholder:text-white/30" />
            ...
            <div className="mb-5">
              <label style={{ color: isDark ? '#a89ef8' : '#534AB7' }} className="block text-[11px] font-semibold uppercase tracking-wider mb-1.5">Confirm password</label>
              <input type="password" value={confirm} onChange={e => setConfirm(e.target.value)} placeholder="Repeat password" required style={{ background: isDark ? 'rgba(255,255,255,0.06)' : '#f0eeff', color: isDark ? 'rgba(255,255,255,0.9)' : '#1a1340', border: confirm && password!==confirm ? '1px solid #E24B4A' : (isDark ? '1px solid rgba(255,255,255,0.10)' : '1px solid rgba(127,119,221,0.2)') }} className="w-full p-[12px_16px] rounded-2xl text-[14px] outline-none transition-all focus:border-indigo-400 focus:ring-2 focus:ring-indigo-400/20 placeholder:text-slate-400 dark:placeholder:text-white/30" />
            ...
            {error && (
              <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 rounded-xl p-3 mb-4 text-[12px] text-red-600 dark:text-red-400 text-center">
                {error}
              </div>
            )}

            <button type="submit" disabled={busy} className="w-full p-[14px] rounded-2xl border-none bg-gradient-to-br from-indigo-500 to-indigo-700 text-white text-[15px] font-bold flex items-center justify-center gap-2.5 transition-all hover:opacity-90 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_8px_24px_rgba(127,119,221,0.3)] disabled:shadow-none hover-lift">
              {busy ? (
                <>
          ...
          <p className="text-center text-[13px] text-slate-500 dark:text-white/50 mt-5">
