...
export async function POST(req: Request) {
  try {
    ...
    doc.setFont('helvetica', 'normal');
    const ugFees = [
      { label: 'Annual Tuition Fee', amount: `Rs. ${college.fees_structure.ug_annual.toLocaleString('en-IN')}` },
      { label: 'Total 4-Year Fee', amount: `Rs. ${college.fees_structure.ug_total.toLocaleString('en-IN')}` },
      { label: 'Hostel Fee (Annual)', amount: college.fees_structure.hostel_annual ? `Rs. ${college.fees_structure.hostel_annual.toLocaleString('en-IN')}` : 'N/A' },
    ];
    ...
    if (college.level === 'Both' || college.level === 'PG') {
      ...
      doc.setFont('helvetica', 'normal');
      const pgFees = [
        { label: 'Annual Tuition Fee', amount: college.fees_structure.pg_annual ? `Rs. ${college.fees_structure.pg_annual.toLocaleString('en-IN')}` : 'N/A' },
        { label: 'Total 2-Year Fee', amount: college.fees_structure.pg_total ? `Rs. ${college.fees_structure.pg_total.toLocaleString('en-IN')}` : 'N/A' },
      ];
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    doc.text(`Average Package: Rs. ${college.avg_package_lpa} LPA`, 25, yPos);
    yPos += 7;
    doc.text(`Highest Package: Rs. ${college.max_package_lpa} LPA`, 25, yPos);
    yPos += 7;
    ...
    doc.text('Note: This is not confirm fee structure and kindly contact with the admission office or contact the support portal.', 105, yPos, { align: 'center' });
    yPos += 5;
