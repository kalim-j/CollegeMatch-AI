...
export default function DiscoverPage() {
  ...
  const handleStart = () => { setDirection(1); setCurrentStep(1); };

  const handleOptionSelect = (questionId: number, optionId: string, streams: string[]) => {
    setAnswers(prev => ({ ...prev, [questionId]: { optionId, streams } }));
  };
    ...
    if (currentStep < 10) {
      ...
      try {
        ...
        const res = await fetch('/api/discover-stream', {
          ...
          body: JSON.stringify({ answers: answersArray, studentName: user.displayName || 'Student' }),
        });
        ...
        if (data.results) {
          ...
          await setDoc(newDocRef, { timestamp: serverTimestamp(), answers: answersArray, results: data.results, selectedStream: null });
          setDiscoveryId(newDocRef.id);
          setCurrentStep(12);
        }
      } catch (err: unknown) {
        setAiError(err instanceof Error ? err.message : 'AI is temporarily unavailable.');
        setCurrentStep(10);
      }
    }
  };
    ...
    <div style={{ minHeight: '100vh', background: bgPage, display: 'flex', flexDirection: 'column', padding: '0 16px 80px', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: 720, width: '100%', margin: '0 auto', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <AnimatePresence mode="wait" custom={direction}>
          ...
          >
            <div style={{ background: bgCard, backdropFilter: 'blur(24px)', border: `1px solid ${border}`, borderRadius: 24, padding: '36px 32px 32px', boxShadow: '0 8px 32px rgba(0,0,0,0.06)' }}>
              ...
              <div style={{ textAlign: 'center', marginBottom: 28 }}>
                ...
                <p style={{ fontSize: 14, color: txtMuted, fontStyle: 'italic' }}>{question.subtitle}</p>
              </div>
                ...
                {question.options.map((option, oIdx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Text */}
                      <div style={{ flex: 1 }}>
                        <h4 style={{ fontSize: 14, fontWeight: 700, color: isSelected ? cat.from : txtPri, marginBottom: 4, lineHeight: 1.3 }}>
                          {option.label}
                        </h4>
                        <p style={{ fontSize: 12, color: txtMuted, lineHeight: 1.4, margin: 0 }}>
                          {option.sub}
                        </p>
                      </div>
      ...
      <div style={{ maxWidth: 720, width: '100%', margin: '20px auto 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <motion.button
          ...
          whileHover={hasAnswered ? { scale: 1.04, boxShadow: `0 8px 28px ${cat.shadow}` } : {}}
          whileTap={hasAnswered ? { scale: 0.96 } : {}}
          style={{
            padding: '12px 32px', borderRadius: 14, border: 'none',
            background: hasAnswered ? `linear-gradient(135deg, ${cat.from}, ${cat.to})` : border,
            color: hasAnswered ? '#fff' : txtMuted,
            fontWeight: 800, cursor: hasAnswered ? 'pointer' : 'not-allowed',
            fontSize: 15, display: 'flex', alignItems: 'center', gap: 8,
            boxShadow: hasAnswered ? `0 4px 20px ${cat.shadow}` : 'none',
            transition: 'background 0.3s, box-shadow 0.3s',
          }}
        >
          {currentStep === 10 ? 'See my stream ✨' : 'Next'} <i className="ti ti-arrow-right" />
        </motion.button>
      </div>
    </div>
  );
